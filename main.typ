...
`bind` 其实已经能实现简单的编程语言了。

想象我们有三个盒子 $A, B, C$，下面是一个用于操作盒子里的数的编程语言 HeLang。

为方便起见，规定只能写 1 行初始化和 3 行加法程序：
...
=== 解释
- 初始化：$A$ 中是 2，$B$ 中是 3，$C$ 中是 7
- 给 $A$ 中数字加上 $B$ 中数字，得到 `[ 5 | 3 | 7 ]`
- 给 $B$ 中数字加上 $A$ 中数字，得到 `[ 5 | 8 | 7 ]`
- 给 $C$ 中数字加上 $B$ 中数字，得到 `[ 5 | 8 | 15 ]`
])

处理的目标是打印出 $A, B, C$ 的最终值。假设我们有函数处理单个字符、空格和换行、盒子编号 $A, B, C$ 和数字，你可以拼出整个处理 HeLang 的函数吗？

暂时想不到的话没关系，两个小节后我将继续讲解这个案例。
] 
