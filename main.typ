...
#sect(title: "你可能会问：编程语言是什么？", color: "red")[
你手机里的天气预报能预测 2 小时内下不下雨，是因为有人用编程语言告诉某台机器应该怎么预测；各种 APP 能验证你验证码输入是否正确，是因为有人用编程语言告诉某台机器应该怎么判断……

若令机器把某种人类能看懂的「任务描述」转化成「执行结果」，就可以认为实现了一个编程语言。暂时不知道编程语言具体长啥样没关系，本 Talk 后面会有不少例子。

本次 Talk 中我将通过编程来实现编程语言。
] 
...
#sect(title: "补充：如果你已经会命令式编程……", color: "blue")[
函数式编程里很多概念和命令式编程相似而不同，请注意避免混淆。
] 
...
有了这些定义，就可以把 `plus` 的几个例子用 Haskell 代码写出来：
...
刚刚我们看到了 Haskell 代码的样子。要处理这种代码，我们首先需要不少小零件，例如：
...
#table(
  ...
  align: horizon,
  [零件], [作用], [零件成功例子], [零件爆炸例子],
  [`char x`], [判断下个字符是否是 $x$], [`char 'm' "maimai" = Success () "aimai"`], [`char 'm' "arcaea" = Fail`],
  [`exact s`], [判断是否以文本 $s$ 开头], [`exact "mai" "maimai" = Success () "mai"`], [`exact "mai" "manjaro" = Fail`],
  [`num`], [读入一个数字], [`num "9TH5IN" = Success 9 "TH5IN"`], [`num "c.s.q.n." = Fail`],
  [`ident`], [读入一个合法名字], [`ident "Au5!" = Success "Au5" "!"`], [`ident "&34 is a cat" = Fail`]
)
...
- 为空，例如 `""`
- 由一个字符和一个字符串拼接而成，例如 `"Genshin"` 由 `'G'` 和 `"enshin"` 拼接而成
...
- 计划用 $"num"$ 判断当前字符串，让 $"bind"$ 帮你判断，你开始摆烂睡觉，成功求值后 $"bind"$ 再叫醒你
  - 若你被用值 $a$ 叫醒了，让 $"bind"$ 再帮你用 $"char"(\'"+"\')$ 判断剩余字符串，继续睡觉
    - 若你被叫醒了，你不在乎值是什么，让 $"bind"$ 再帮你用 $"num"$ 判断，继续睡觉
      - 若你被用值 $b$ 叫醒了，得到值 $a + b$
$"bind"$ 如果爆炸就不能叫醒你了，直接 `Fail`。
])]

#sect(title: [你可能会问：代码中的 `\` 是什么？], color: "red")[
`\` 的实质作用是在函数里面再定义一个新函数，例如：

- `f x = x + 1` 也可以写成 `f = \x => x + 1`，接受 `x`，返回 `x + 1`
- `add x y = x + y` 也可以写成 `add x = \y => x + y`
- `bind num (\a => ...)` 的第二个参数就是个函数，接受上一步的值，返回 `...` 的内容

由于这样定义出来的函数是「匿名」的，我们也把它成为「匿名函数」。
] 
...
是不是比开头短了好多好多？这就是为什么 $"bind"$ 是神。

#sect(title: [你可能会问：$"bind"$ 怎么处理 $1+1$ 以外的事物？], color: "red")[
还记得前面讲的匿名函数吗？`\x => y + z` 这种。

要处理这种匿名函数，暂时不考虑空格，只允许右边是变量加变量，我们可以这样组合：
...
处理一个匿名函数的过程是：
- 读入 `\`，睡一觉
- 用 $"ident"$ 读取参数名，睡一觉，得到 $x$
- 读入 `=>`，睡一觉
- 用 $"ident"$ 读取第一个变量名，睡一觉，得到 $y$
- 读入 `+`，睡一觉
- 用 $"ident"$ 读入第二个变量名，睡一觉，得到 $z$
- 结果是 $"Func"(x)(y)(z)$，暂时规定它用于表示 `\x => y + z`
])

结合后面讲的「二选一」，我们将不再只能处理这种固定格式的代码，可以变得更灵活。
] 

然而要把 $"bind"$ 真的请到 Haskell 中，我们首先需要确定其类型：
